...
def product_trades_per_year(session, export_path, batch_size=10):
    """Generate a JSON file with product trades per year."""
    dir_path = os.path.abspath(export_path)
    os.makedirs(dir_path, exist_ok=True)
    
    print("Starting export of product trades per year...")
    ...
    with tqdm(total=len(products), desc="Exporting products", unit="product") as pbar:
        for batch_idx in range(0, len(products), batch_size):
            ...
            files_written = 0
            for row in result:
                year, product_code, trade_data_json = row
                file_name = f"{year}_{product_code}.json"
